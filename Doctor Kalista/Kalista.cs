...
namespace Kalista
{
    internal class Kalista
    {
        ...
        private static void OnLoad(EventArgs args)
        {
            ...
            Chat.Print("Kalista Loaded.Good Luck");
            Q.MinimumHitChance = HitChance.AveragePoint;
            KalistaMenu.InitMenu();
            ...
            if (!Player.IsDead)
            {
                if (CanMoveTick <= Environment.TickCount + 100)
                {
                    ...
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo))
                    {
                        ...
                        if (KalistaMenu.GetBoolValue(KalistaMenu.SpellsMenu, "use.q.combo"))
                        {
                            if (Q.Ready())
                            {
                                if (!Player.IsDashing())
                        ...
                        if (KalistaMenu.GetBoolValue(KalistaMenu.SpellsMenu, "use.e"))
                            if (E.Ready())
                                if (
                                    EntityManager.Heroes.Enemies.Any(
                                        x =>
                    ...
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Harass))
                    {
                        if (KalistaMenu.GetBoolValue(KalistaMenu.SpellsMenu, "use.q.harass") &&
                            Player.ManaPercent < KalistaMenu.GetSliderValue(KalistaMenu.SpellsMenu, "manapercent") &&
                    ...
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.LaneClear))
                    {
                            ...
                            Player.ManaPercent > KalistaMenu.GetSliderValue(KalistaMenu.SpellsMenu, "manapercent"))
                        {
                                                    ...
                                                    DamageType.Physical, Q.Range)))
                            {
                                ...
                                if (killableNumber >=
                                    KalistaMenu.GetSliderValue(KalistaMenu.LaneClear, "q.min.kill"))
                                {
                                    if (!Orbwalker.IsAutoAttacking)
                                    {
                        ...
                        //E Laneclear Logic
                        if (E.Ready() &&
                            KalistaMenu.GetSliderValue(KalistaMenu.LaneClear, "manapercent") < Player.ManaPercent)
                        {
                            if (
                                EntityManager.MinionsAndMonsters.GetLaneMinions(EntityManager.UnitTeam.Enemy,
                                    Player.Position, E.Range).Count(x =>
                                        ...
                                        x.IsKillableAndValidTarget(Utility.GetRendDamage(x),
                                            DamageType.Physical)) >=
                                KalistaMenu.GetSliderValue(KalistaMenu.LaneClear, "e.minkill"))
                            {
                                E.Cast();
                            }
                        }
                    ...
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.JungleClear) &&
                        KalistaMenu.GetSliderValue(KalistaMenu.JungleClear, "manapercent") < Player.ManaPercent)
                    {
                        //Q logic
                        if (KalistaMenu.GetBoolValue(KalistaMenu.JungleClear, "use.q") && Q.Ready())
                        {
                        ...
                        //E logic
                        if (KalistaMenu.GetBoolValue(KalistaMenu.JungleClear, "use.e") && E.Ready())
                        {
                            if (
                ...
                //Perma Active
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.killsteal") && E.Ready())
                {
                ...
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.mobsteal") && E.Ready())
                {
                    if (
                        EntityManager.MinionsAndMonsters.GetJungleMonsters(Player.Position, E.Range)
                ...
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.siegeandsuper") && E.Ready())
                {
                    if (
                ...
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "r.balista") && R.Ready())
                {
                    var blitz =
                ...
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.harass") &&
                    !KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.dontharasscombo") && E.Ready() &&
                ...
                if (KalistaMenu.Misc["w.castdragon"].Cast<KeyBind>().CurrentValue && W.Ready())
                {
                    if (Player.Distance(DragonLocation) <= W.Range)
                    {
                        W.Cast(DragonLocation);
                ...
                if (KalistaMenu.Misc["w.castbaron"].Cast<KeyBind>().CurrentValue && W.Ready())
                {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            if (!Player.IsDead)
            {
                if (KalistaMenu.GetBoolValue(KalistaMenu.Drawings, "draw.q") && Q.Ready())
                    Drawing.DrawCircle(Player.Position, Q.Range,
                        Color.DarkBlue);

                if (KalistaMenu.GetBoolValue(KalistaMenu.Drawings, "draw.w") && W.Ready())
                    Drawing.DrawCircle(Player.Position, W.Range,
                        Color.DarkBlue);

                if (KalistaMenu.GetBoolValue(KalistaMenu.Drawings, "draw.e") && E.Ready())
                    Drawing.DrawCircle(Player.Position, E.Range,
                        Color.DarkBlue);

                if (KalistaMenu.GetBoolValue(KalistaMenu.Drawings, "draw.r") && R.Ready())
                    Drawing.DrawCircle(Player.Position, R.Range,
                        Color.DarkBlue);


                if (KalistaMenu.GetBoolValue(KalistaMenu.Drawings, "draw.e.dmgpercent"))
                {
        ...
        private static void Orbwalking_OnNonKillableMinion(Obj_AI_Base minion, Orbwalker.UnkillableMinionArgs args)
        {
            ...
            {
                if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.lasthit.assist") && E.Ready() && !EntityManager.Heroes.Enemies.Any(x => Player.IsInAutoAttackRange(x)))
                    foreach (
        ...
        private static void Obj_AI_Base_OnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
            if (sender != null && args.Target != null)
                if (sender.IsEnemy)
                    ...
                    {
                        if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "r.savebuddy"))
                            if (R.Ready())
                            {
                                var soulbound =
                                    EntityManager.Heroes.Allies.FirstOrDefault(
                        ...
                        if (KalistaMenu.GetBoolValue(KalistaMenu.Misc, "e.beforedie"))
                            if (args.Target.IsMe)
