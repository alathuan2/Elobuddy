...
namespace Kalista.Utility
{
    ...
    public class RecallTracker : UtilityAddon
    {
        ...
        {
            Menu.AddGroupLabel("Kalista - RecallTracker for " + Player.Instance.ChampionName, "recall.grouplabel.utilitymenu");
        ...
        protected override void Drawing_OnEndScene(EventArgs args)
        {
            if (Value.Use("recall.use"))
            {
                ...
                {
                   KalistaGeometry.DrawRect(BarX, BarY, BarWidth, _barHeight, 1, Color.DarkGray);
                }
                else
                {
                    return;
                ...
                foreach (var recall in recalls)
                {
                    if (recall.Status == RecallStatus.Active)
                    {
                        ...
                        KalistaGeometry.DrawRect(BarX, BarY, BarWidth * (float)percent, _barHeight, 1, colorBar);
